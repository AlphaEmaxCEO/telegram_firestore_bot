...
def submit_product(update: Update, context: CallbackContext):
    user_id = str(update.effective_user.id)
    args = context.args
    if not args or len(args) < 2:      update.message.reply_text("Usage: /submit_product <ProductName> <Price>")
        return
    product_name = args[0]
    try:
        price = float(args[1])
    except ValueError:       update.message.reply_text("Price must be a number.")
        return
...
def pay_listing(update: Update, context: CallbackContext):
    user_id = str(update.effective_user.id)
    args = context.args
    if not args:        update.message.reply_text("Usage: /pay_listing <ProductName>")
        return
    product_name = args[0]
    ...
    if balance < product['listing_fee']:
        ...
        return
    wallet_ref.set({"balance": balance - product['listing_fee']})  product_doc.reference.update({"status": "pending_approval"})
...
def approve_product(update: Update, context: CallbackContext):
    ...
    if admin_id != OWNER_ID and admin_id != ASSISTANT_ID:
       update.message.reply_text("🚫 You are not authorized.")
        return
    args = context.args
    if not args:       update.message.reply_text("Usage: /approve_product <ProductName>")
        return
    product_name = args[0]
  products_ref = db.collection('products') \
    ...
    if not products_ref:
       update.message.reply_text("❌ No pending product found.")
        return
    product_doc = products_ref[0] product_doc.reference.update({"status": "approved"})
...
def deny_product(update: Update, context: CallbackContext):
    ...
    if admin_id != OWNER_ID and admin_id != ASSISTANT_ID:
       update.message.reply_text("🚫 You are not authorized.")
        return
    args = context.args
    if not args:   update.message.reply_text("Usage: /deny_product <ProductName>")
        return
    product_name = args[0]
    products_ref = db.collection('products') \
        .where('product_name', '==', product_name) \
    ...
    if not products_ref:
       update.message.reply_text("❌ No pending product found.")
        return
    product_doc = products_ref[0]
    seller_id = product_doc.to_dict()['user_id'] product_doc.reference.update({"status": "denied"})
...
def main():
    ...
    dp = updater.dispatcher   dp.add_handler(CommandHandler("start", start))    dp.add_handler(CommandHandler("balance", balance))    dp.add_handler(CommandHandler("submit_product", submit_product, pass_args=True))    dp.add_handler(CommandHandler("pay_listing", pay_listing, pass_args=True))   dp.add_handler(CommandHandler("approve_product", approve_product, pass_args=True))    dp.add_handler(CommandHandler("deny_product", deny_product, pass_args=True))
